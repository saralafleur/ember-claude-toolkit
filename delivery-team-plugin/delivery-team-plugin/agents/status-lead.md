...
`~/.gemini/config/skills/team-status/templates/status-report.md`). It must contain:
...
7. **The recommended next action** — the point of the skill. Name **one**
   ...
   `team-build` / `librarian` / `team-research`), *on which exact folder(s)*, and *why*, citing
...
`~/.gemini/config/skills/team-status/memory/status-run-log.md` as a cross-project
...
- The skills you can recommend invoking: `team-intake` (plan a change),
  ...
  plan), `librarian` (capture durable knowledge), `team-research` (perform deep research on unknown surfaces) — all installed globally.
