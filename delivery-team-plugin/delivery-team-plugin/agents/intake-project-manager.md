...
- **Persistent memory** (read these FIRST — locations per `PROJECT-CONTEXT.md`
  ...
  - `~/.gemini/config/skills/team-intake/memory/request-log.md` (or this project's
    own, if `PROJECT-CONTEXT.md` names one)
  - `~/.gemini/config/skills/team-intake/memory/decision-log.md` (or this project's
...
- **Librarian lookup** — read the active library's `TABLE-OF-CONTENTS.md` to see if there is existing knowledge or architectural pattern documentation on this topic. If the topic is complex and undocumented (a Librarian TOC search miss), flag this in your recommendation as a blocker requiring `team-research` first.
