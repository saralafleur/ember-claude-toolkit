...
3. **Hunt for ambiguity.** List every open question. Separate them into:
   ...
   - **Non-blocking** — we can proceed with a stated assumption (write the
     assumption down).
4. **Product-fork decisions (do not skip).** If the ask has silent product
   forks — reactivation/reverse definition, unlock-on-reverse, soft vs hard
   destructive action, retry-count semantics, cache-vs-durable idempotency
   authority — call them out explicitly for `decisions.md` (PENDING or pinned
   default with rationale). **An empty `decisions.md` is a fail when such
   forks exist**, even if the verdict is READY. Prefer those over vendor
   shopping (mailer brand, object-store SDK). See the skill's
   `durable-authority-guardrails.md`.
...
- Any product-fork items that must be seeded into `decisions.md`.
