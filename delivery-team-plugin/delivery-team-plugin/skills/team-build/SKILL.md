...
> install (`~/.gemini/config/skills/team-build/` + `~/.gemini/config/agents/`). If you
> installed this as a plugin instead, every `~/.gemini/config/skills/team-build/...`
...
- **Templates:** `~/.gemini/config/skills/team-build/templates/`.
- **Memory:** the build run-log location comes from `PROJECT-CONTEXT.md` if
  the project names one; otherwise falls back to
  `~/.gemini/config/skills/team-build/memory/build-run-log.md` (a cross-project log —
