...
> install (`~/.gemini/config/skills/team-release/` + `~/.gemini/config/agents/`). If you
> installed this as a plugin instead, every `~/.gemini/config/skills/team-release/...`
...
- Links to `release-notes.md` (the deliverable) and `release-crosswalk.md` (internal).
- **Librarian Capture Recommended**: Advise the user to run the **`librarian`** skill in `capture` mode to harvest project-wide lessons, codebase patterns, or setup quirks resolved during this release cycle into the shared project library.
...
- **Output per release:** `<release-docs-root>/<version>/` (per
  ...
  `decisions.md`.
- **Templates:** `~/.gemini/config/skills/team-release/templates/`.
- **Memory:** the release-log location comes from `PROJECT-CONTEXT.md` if the
  project names one; otherwise
  `~/.gemini/config/skills/team-release/memory/release-log.md` (a cross-project
