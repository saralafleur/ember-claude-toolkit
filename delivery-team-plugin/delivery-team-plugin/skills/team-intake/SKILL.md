...
> install (`~/.gemini/config/skills/team-intake/` + `~/.gemini/config/agents/`). If you
> installed this as a plugin instead, every `~/.gemini/config/skills/team-intake/...`
...
> role brief from `~/.gemini/config/agents/<name>.md`.)
...
- **Log every clarifying/blocking question and its answer** (see "Decision
  ...
  proceed.
- **Product-fork rule:** if the ask has silent product forks (see
  `durable-authority-guardrails.md`), `decisions.md` must not stay empty —
  seed PENDING entries or pin defaults with rationale. Prefer those over
  vendor shopping.
...
When the request involves webhooks, idempotency, retry/DLQ, or delayed
destructive work, tell architect/engineer/QA to apply
`durable-authority-guardrails.md` (PASS/FAIL/N/A per trap).
...
- **Librarian Integration (Knowledge Retrieval & Research Trigger)**: The PM/Tech Lead queries the Librarian's Table of Contents (`TABLE-OF-CONTENTS.md` in the active library root) to check for relevant global conventions, architectural guidelines, or lessons learned. If the query yields a **miss** on a new technology, API, or pattern, or if triage flags technical uncertainty, they recommend triggering `team-research` to investigate the blind spot and archive findings in the library first.
...
**Empty-log fail:** if the request has silent product forks listed in
`durable-authority-guardrails.md`, an empty `decisions.md` is not acceptable
— even when triage is `READY` and defaults are pinned. Record the pin.
...
- **Output per request:** `<intake-base>/intake/<date>-<slug>/` containing
  ...
  `supporting/*.md`.
- **Templates:** `~/.gemini/config/skills/team-intake/templates/`.
- **PM memory:** location comes from `PROJECT-CONTEXT.md` if the project
  names one; otherwise `~/.gemini/config/skills/team-intake/memory/` (a
  ...
  append-only).
- **Durable authority:** `durable-authority-guardrails.md` beside this skill
  — standing traps for webhook/idempotency/deferred-destruction designs.
