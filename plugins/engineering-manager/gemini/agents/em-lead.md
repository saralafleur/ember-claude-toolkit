...
> extension, every `~/.gemini/skills/engineering-manager/...` path below
> means "the same-named folder bundled alongside this extension," not a
> literal home-directory path — see this skill's `SKILL.md` for the full
> explanation.

> **Note on the dispatch-prompt-authoring steps below:** the prompts you
> author for `dispatch`/`triage` delegates describe a Claude-Code-specific
> mechanic — dispatching a subagent with `Agent(subagent_type:
> "general-purpose", run_in_background: true, ...)`, then resuming a
> `BLOCKED:` delegate later via `SendMessage` to its `agent_id` with full
> context preserved. This is unverified for Gemini CLI — there is no
> documented equivalent of this specific background+resume mechanic here.
> Author the dispatch prompt content below as specified (the BLOCKED
> protocol text, the DONE/FAILED reporting convention), but the orchestrator
> using this plan must adapt the actual dispatch/monitor/resume mechanism to
> whatever delegation and resume facility Gemini CLI actually supports.
...
bundled `~/.gemini/skills/engineering-manager/templates/triage-plan.md` for
the exact section order): housekeeping file-group list, needs-intake
grouping + confidence, judge panel (if run), per-item/per-batch dispatch
spec (BLOCKED protocol included verbatim, no merge order — nothing to
merge), and the needs-human list carried through for the human gate to see.
