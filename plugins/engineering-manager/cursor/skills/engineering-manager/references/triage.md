...
> **Cross-tool note:** the dispatch/monitor/resume mechanics described below
> assume Claude Code's `Agent` tool (`subagent_type`, `run_in_background`,
> and `SendMessage`-to-resume for a `BLOCKED` delegate). This is unverified
> for Cursor — Cursor has no documented equivalent of this specific
> background-dispatch-and-resume mechanic. Adapt to whatever delegation/
> resume mechanism Cursor actually supports when using this.
...
`~/.cursor/skills/engineering-manager/templates/triage-plan.md`): the
...
`<target>/triage-decisions.md` (from
`~/.cursor/skills/engineering-manager/templates/decision-log.md`, create if
...
backgrounded delegate (`Agent(subagent_type: "general-purpose",
run_in_background: true)` in Claude Code — see the cross-tool note at the
top of this file for what to use in Cursor) with a fully self-contained
prompt — the file path(s), the exact old text and the exact new text (or
the fact to substitute) per correction, and an instruction to read the file
first, verify the current text still matches before editing (state may have
moved since the report was written), and report a before/after snippet. No
...
backgrounded delegate per item (or per batch, for BATCHED groups) with a
fully self-contained prompt: the project's root and `PROJECT-CONTEXT.md`
location, the request description, an instruction to write a `request.md`
under a new `<target>/<new-item-slug>/` folder and then invoke the
`team-intake` skill targeting it, and this **BLOCKED protocol**, verbatim
(same shape as `dispatch`'s, adapted — there's no worktree/branch to
reference, just the folder):
...
Launch every member of a PARALLEL group in the same message (multiple
tool calls, one message). For SEQUENTIAL, launch only the first; launch the
next after its predecessor reports DONE. For BATCHED, there's one delegate
per batch, not one per original item — the batch's own `request.md` lists
each original item as a separate ask within one document (see
...
`~/.cursor/skills/engineering-manager/memory/dispatch-run-log.md`) — date ·
