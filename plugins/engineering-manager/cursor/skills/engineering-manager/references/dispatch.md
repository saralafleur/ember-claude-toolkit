...
the entire reason the pipeline keeps those documents authoritative. (See the
cross-tool note at the top of this file — this dispatch mechanic is Claude
Code's; use whatever Cursor's own equivalent delegation call is.)
...
- **`BLOCKED: ...`** — read the stated question. If it's answerable from a
  ...
  unrelated items — get an answer, then resume that delegate with the answer
  (`SendMessage` to its `agent_id` in Claude Code — see the cross-tool note
  at the top of this file for what to use in Cursor). This resumes the
...
`~/.cursor/skills/engineering-manager/memory/dispatch-run-log.md` (create
from `~/.cursor/skills/engineering-manager/templates/run-log-header.md` if
