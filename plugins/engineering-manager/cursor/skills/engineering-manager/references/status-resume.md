...
2. **Same-session resume (preferred, if the agent ID is still live in this
   conversation)**: resume that delegate with the given answer (`SendMessage`
   to its `agent_id` in Claude Code — see the cross-tool note at the top of
   this file for what to use in Cursor). This continues the exact delegate
   with full context — worktree state (for `dispatch`) or whatever intake
   artifacts it already drafted (for `triage`), partial progress, which step
   it paused on. Update the relevant state file back to `IN_PROGRESS`.
3. **Cross-session fallback (if the original agent ID isn't resumable — a
   ...
   - **For a `dispatch` entry**: the delegate already wrote the open
     ...
     asked and decided), then dispatch a **fresh** backgrounded delegate
     with the same kind of self-contained prompt as the original dispatch:
     run `team-build` on the item's folder. Because the worktree, branch,
     and now-answered decision are all already on disk, this fresh delegate
     picks up essentially where the paused one left off rather than
     starting the build over — it is a new agent, not new work.
   - **For a `triage` intake entry**: there's no `decisions.md` to have
     ...
     **fresh** backgrounded delegate whose prompt includes the original
   ...
   - Either way, record the new delegate's id in the relevant state file,
