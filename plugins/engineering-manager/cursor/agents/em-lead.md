...
> `~/.cursor/skills/engineering-manager/...` path below means "the same-named
> folder bundled alongside this plugin," not a literal home-directory path —
> see this skill's `SKILL.md` for the full explanation.
...
3. **Draft the concrete per-item (or per-batch) dispatch spec** for every
   ...
   > **Cross-tool note:** the dispatch/monitor/resume mechanics described
   > below (and in `references/dispatch.md`, `references/triage.md`, and
   > `references/status-resume.md`) assume Claude Code's `Agent` tool
   > (`subagent_type`, `run_in_background`, and `SendMessage`-to-resume for a
   > `BLOCKED` delegate). This is unverified for Cursor — Cursor has no
   > documented equivalent of this specific background-dispatch-and-resume
   > mechanic. Adapt to whatever delegation/resume mechanism Cursor actually
   > supports when using this; don't assume the mechanics below transfer
   > as-is.

   The shape differs by which command you're running for:
...
**For `triage`, write `<target>/triage-plan.md`** (see
`~/.cursor/skills/engineering-manager/templates/triage-plan.md` for the
exact section order): housekeeping file-group list, needs-intake grouping +
