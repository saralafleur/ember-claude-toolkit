...
> **Note:** the resume mechanics described below — same-session resume via
> `SendMessage` to a delegate's `agent_id`, and the cross-session fallback of
> dispatching a fresh `Agent(subagent_type: "general-purpose",
> run_in_background: true)` delegate — are Claude Code's `Agent` tool
...
2. **Same-session resume (preferred, if the delegate is still live in this
   conversation)**: send the given answer to that delegate. This continues
   the exact delegate with full context — worktree state (for `dispatch`) or
   whatever intake artifacts it already drafted (for `triage`), partial
   progress, which step it paused on. Update the relevant state file back to
   `IN_PROGRESS`.
3. **Cross-session fallback (if the original delegate isn't resumable — a
   ...
   - **For a `dispatch` entry**: the delegate already wrote the open
     ...
     asked and decided), then dispatch a **fresh** backgrounded,
     full-tool-access delegate with the same kind of self-contained prompt
     as the original dispatch: run `team-build` on the item's folder.
     Because the worktree, branch, and now-answered decision are all
     already on disk, this fresh delegate picks up essentially where the
     paused one left off rather than starting the build over — it is a new
     delegate, not new work.
   - **For a `triage` intake entry**: there's no `decisions.md` to have
     ...
     **fresh** backgrounded, full-tool-access delegate whose prompt includes
     the original request plus the now-answered question and its answer, so
     it doesn't re-ask. If the delegate had already gotten as far as writing
