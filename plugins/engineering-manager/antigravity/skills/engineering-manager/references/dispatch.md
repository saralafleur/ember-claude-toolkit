...
Each delegate is launched as a backgrounded, full-tool-access delegate with
the exact dispatch prompt from `dispatch-plan.md`. Never hand it this
session's own conversation history — the delegate needs none of it, only the
item's own on-disk plan/decisions, which is the entire reason the pipeline
keeps those documents authoritative.
...
{
  "<item-slug>": {
    "agent_id": "<from the delegate-launch result>",
    "status": "IN_PROGRESS",
...
- **`BLOCKED: ...`** — read the stated question. If it's answerable from a
  ...
  defaulted," so it already auto-decided everything it safely could before
  escalating. There's nothing left to auto-decide here.
  ...
  answer. This resumes the **same** delegate with full context; it is not a
  restart. Mark the item `BLOCKED` → back to `IN_PROGRESS` in
  `dispatch-state.json` once resumed.
...
note) — create from `templates/run-log-header.md` if
