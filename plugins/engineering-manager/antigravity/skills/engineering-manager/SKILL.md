...
> rooted at `~/.gemini/config/plugins/engineering-manager/`; if your actual
> install differs, treat every path below as relative to this plugin's own
> bundled `skills/engineering-manager/...` folder instead.
...
> **How to invoke each role:** these are registered subagents. Delegate to
> one either by explicit mention — `@em-analyst <task>` — or by letting
> automatic delegation match the task to the subagent's `description`
> field. (If a subagent by that name isn't available, fall back to a plain
> instruction and paste the role brief from
> `~/.gemini/config/plugins/engineering-manager/agents/<name>/agent.md`
...
## Why delegates use a general-purpose, full-tool-access role

The narrow role agents (`build-triage`, `build-implementer`,
`intake-triage`, `intake-tech-lead`, …) have no ability to spawn further
subagents. A delegate that's going to run a **skill** itself (`team-build`
for `dispatch`, `team-intake` for `triage`'s intake phase — invoking each of
that skill's own roles in turn) needs full tool access, so both commands
always dispatch a general-purpose delegate with an instruction to run the
named skill — never one of the specialist role agents directly, and never a
context-inheriting fork (a fork would inherit this entire session's history,
which the delegate doesn't need and shouldn't pay to load — see
...
further delegation ability of their own.
...
(See the note at the top of `references/dispatch.md` on how this mechanic
is unverified for Antigravity and needs adapting to whatever this platform
actually supports.)
