...
_This agent reads and searches files, and writes only its own plan document (`dispatch-plan.md` or `triage-plan.md`); it does not edit any other file, dispatch anything, or touch product code._
...
3. **Draft the concrete per-item (or per-batch) dispatch spec** for every
   ...
   > **Note:** the dispatch prompts below (the BLOCKED/DONE/FAILED protocol
   > and the "resume by sending a message to the delegate's agent ID" idea)
   > describe Claude Code's `Agent` tool mechanics —
   > `subagent_type`/`run_in_background`/`SendMessage`-to-resume. This is
   > **unverified for Antigravity**; adapt the dispatch/monitor/resume
   > wording below to whatever delegation and resume mechanism Antigravity
   > actually supports before authoring a real dispatch prompt.
