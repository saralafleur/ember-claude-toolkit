...
---
<!-- assumption: Copilot custom-agent format is in Preview. `model:` omitted so the subagent inherits the user's selected Copilot model — the source defined none. Source `tools:` was Read, Grep, Glob, Bash (no Write/Edit) — mapped to 'codebase'+'search'+'runCommands' only, so this agent cannot write any file. -->
<!-- assumption: this agent is invoked 2-3x in parallel per the source design (a judge panel). Copilot's custom-agent Preview format has no documented multi-instance-of-one-agent invocation primitive verified for this target — invoke it multiple times via whatever parallel-delegation mechanism the runtime actually supports. -->
