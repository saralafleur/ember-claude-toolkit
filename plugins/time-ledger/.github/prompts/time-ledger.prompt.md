---
name: time-ledger
description: Cross-project time, token, and cost ledger built from Claude Code session transcripts. Regenerates the hours/token/cost rollups and rebuilds the HTML dashboard. Invoke with /time-ledger when you want to "update my hours report" or "regenerate the time dashboard".
argument-hint: (no arguments — regenerates the full rollup and dashboard)
tools: ['codebase', 'search', 'runCommands', 'editFiles']
<!-- assumption: Copilot Agent Plugins prompt frontmatter is still in flux (Preview). Fields used here — name, description, argument-hint, tools — follow the current .prompt.md convention; `tools` ids match the convention used across this repo's other Copilot ports (codebase/search for read+search, runCommands for terminal execution, editFiles for write/edit). Adjust to the exact tool ids your Copilot build exposes. -->
---
...
Scripts for this plugin are self-contained under `.github/scripts/` (relative
to this plugin root), with the SDLC journey HTML template under
`.github/templates/`. Run the Python scripts from the terminal.

> **Non-Claude-Code port note:** `~/.claude/projects` is Claude Code's own
> transcript location. This tool does not write session transcripts there, so
> leaving the default will silently produce empty rollups. Set the
> `TIME_LEDGER_PROJECTS_DIR` environment variable to point at wherever the
> installed tool actually stores its session transcripts — this varies by
> tool, so the skill cannot assume it.
...
Run from this plugin's `.github/scripts/` directory (the Python scripts are
plain and portable — no arguments needed for the rollup pass):
...
Then re-embed the data into the dashboard at the same file path:
...
- **`journey_report.py --initiative-root <intake-dir> --title ... --project
  ...
  lifecycle, republished in place as each stage completes. It resolves its
  HTML template from `../templates/sdlc-journey-template.html` relative to the
  script — kept alongside under `.github/templates/`.
