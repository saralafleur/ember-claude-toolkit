...
running shell commands, and writing both the QA assessment and the run-log
memory update.
...
- **QA run-log:** location per `PROJECT-CONTEXT.md` if configured, else
  `~/.gemini/config/plugins/delivery-team/skills/team-qa/memory/qa-run-log.md` (past QA runs).
...
`~/.gemini/config/plugins/delivery-team/skills/team-qa/templates/qa-assessment.md`) with:
...
- Append a row to the QA run-log (location per `PROJECT-CONTEXT.md`, else the
  global fallback at `~/.gemini/config/plugins/delivery-team/skills/team-qa/memory/qa-run-log.md`) — date,
