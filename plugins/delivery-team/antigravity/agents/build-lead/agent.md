...
- Build run-log location: check `PROJECT-CONTEXT.md`; if it names one, append
  there. Otherwise use `~/.gemini/config/plugins/delivery-team/skills/team-build/memory/build-run-log.md`
  (a cross-project fallback — less useful than a project-specific log, but
  better than nothing).

## What to produce
Write `<output-dir>/build-report.md` (template:
`~/.gemini/config/plugins/delivery-team/skills/team-build/templates/build-report.md`) — the document the
user reads:
