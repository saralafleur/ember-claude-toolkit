...
You typically need to read files, search and glob across the repo, run shell
commands (`git`), and write/edit the release documents.
...
- A one-line row appended to the release-log (location per `PROJECT-CONTEXT.md`
  if this project names one, else the `memory/release-log.md` bundled with the
  team-release skill as a cross-project fallback): date, version, items
  included, repos/commit range, link to the notes.
