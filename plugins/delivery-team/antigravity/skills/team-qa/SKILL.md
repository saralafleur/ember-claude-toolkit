...
## The team (first-class agents, installed at `~/.gemini/config/plugins/delivery-team/agents/`)
| Agent | Role |
|-------|------|
...
> install (`~/.gemini/config/plugins/delivery-team/skills/team-qa/` + `~/.gemini/config/plugins/delivery-team/agents/`). If you
> installed this as a plugin instead, every `~/.gemini/config/plugins/delivery-team/skills/team-qa/...`
> path below means "the same-named folder bundled alongside this `SKILL.md`",
> and `~/.gemini/config/plugins/delivery-team/agents/<name>/agent.md` means "the matching agent bundled
> in this plugin's own `agents/<name>/` folder" — same relative layout,
> different root.

> **How to invoke each role:** delegate the role to the matching subagent
> (e.g. "delegate to the qa-coverage-cartographer subagent"). Always give the
> agent: the `change-brief.md` path, the output dir, and (once known) the
> coverage verdict. (If a subagent isn't available, fall back to doing that
> role's work yourself, following the role brief in
> `~/.gemini/config/plugins/delivery-team/agents/<name>/agent.md`.)
...
- **Output per run:** `<base>/qa/<date>-<slug>/` (or `<intake-dir>/qa/`) containing
  ...
  `supporting/*.md`.
- **Templates:** `~/.gemini/config/plugins/delivery-team/skills/team-qa/templates/`.
- **Memory:** the QA run-log location comes from `PROJECT-CONTEXT.md` if the
  project names one; otherwise falls back to
  `~/.gemini/config/plugins/delivery-team/skills/team-qa/memory/qa-run-log.md` (a cross-project log — less
