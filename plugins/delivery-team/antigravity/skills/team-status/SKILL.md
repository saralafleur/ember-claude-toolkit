...
> **Path note:** wherever this file refers to `templates/...` or `memory/...`,
> it means the same-named folder bundled alongside this `SKILL.md`, and each
> agent named above is the matching agent bundled with this extension under
> its own `agents/<name>/` folder.

> **How to invoke each role:** these are the subagents bundled with this
> extension. Delegate to a subagent by name — describe the task and hand it
> to the `<name>` subagent (e.g. delegate to the `status-scanner` subagent).
> Always give the agent: the target folder, the output path for the report,
> and (for scanners) the specific item folder it owns.
...
- **Output per run:** `<target>/status-report.md` (+ transient
  ...
  was logged).
- **Templates:** this extension's own `skills/team-status/templates/`.
- **Memory:** the status run-log location comes from `PROJECT-CONTEXT.md` if
  the project names one; otherwise this extension's own
  `skills/team-status/memory/status-run-log.md` (a cross-project fallback,
  append-only). It has no forked defect-catalog file — it reads the project's
  own if one is configured.
