...
## The team (agents bundled with this skill, under the sibling `agents/` folder)
| Agent | Role |
|-------|------|
...
> **Path note:** the `release-scribe` and `release-lead` role briefs live as
> `.md` files in this plugin's own `agents/` folder; the fill-in templates live
> in this skill's sibling `assets/` folder, and the cross-project release-log
> fallback lives in its sibling `references/` folder.

> **How to invoke each role:** delegate to the named subagent — invoke it
> explicitly with `@release-scribe` / `@release-lead`. Give each: the version
> label, the list of shipped items with their artifact paths, and the output
> dir. (If a name isn't available as a subagent, fall back to running the task
> yourself using the role brief in the sibling `agents/<name>.md`.)
...
- **Output per release:** `<release-docs-root>/<version>/` (per
  ...
  `decisions.md`.
- **Templates:** the sibling `assets/` folder (`release-notes.md`,
  `release-crosswalk.md`).
- **Memory:** the release-log location comes from `PROJECT-CONTEXT.md` if the
  project names one; otherwise the sibling `references/release-log.md` (a
  cross-project fallback, append-only).
