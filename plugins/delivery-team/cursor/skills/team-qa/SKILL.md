...
## The team (custom agents, installed at `~/.cursor/agents/`)
| Agent | Role |
|-------|------|
...
> install (`~/.cursor/skills/team-qa/` + `~/.cursor/agents/`). If you installed
> this as a plugin instead, every `~/.cursor/skills/team-qa/...` path below
> means "the same-named folder bundled alongside this `SKILL.md`" — the
> templates live in this skill's `references/` folder and the run-log memory in
> its `assets/` folder — and `~/.cursor/agents/<name>.md` means "the matching
> file in this plugin's own `agents/` folder" — same relative layout, different
> root.

> **How to invoke each role:** these are registered custom agents. Delegate to
> the matching one explicitly — invoke `@<name>` (e.g. `@qa-coverage-cartographer`)
> or "delegate to the qa-coverage-cartographer subagent". Always give the agent:
> the `change-brief.md` path, the output dir, and (once known) the coverage
> verdict. (If a subagent isn't available in this Cursor build, fall back to
> doing that role's work yourself, following the role brief in
...
1. **Per run:** `<output-dir>/decisions.md` (from `references/decision-log.md`) — the
...
- **Output per run:** `<base>/qa/<date>-<slug>/` (or `<intake-dir>/qa/`) containing
  ...
  `supporting/*.md`.
- **Templates:** `~/.cursor/skills/team-qa/references/`.
- **Memory:** the QA run-log location comes from `PROJECT-CONTEXT.md` if the
  project names one; otherwise falls back to
  `~/.cursor/skills/team-qa/assets/qa-run-log.md` (a cross-project log — less
