...
> install (`~/.cursor/skills/team-intake/` + `~/.cursor/agents/`). If you
> installed this as a plugin instead, every `~/.cursor/skills/team-intake/...`
> path below means "the same-named folder bundled alongside this `SKILL.md`"
> (the supporting files live in this skill's own `assets/` and `references/`
> folders), and `~/.cursor/agents/<name>.md` means "the matching file in this
> plugin's own `agents/` folder" — same relative layout, different root.

> **How to invoke each role:** these are subagents bundled with this plugin
> (`agents/<name>.md`). Delegate to one by name — "delegate to the
> `intake-architect` subagent" — or invoke it explicitly with `@intake-architect
> <task + inputs>`. If delegation isn't available, fall back to running the task
> yourself using the role brief in `agents/<name>.md` as your instructions.
> Always give the agent: the request-brief path, the output dir, and the PM's
...
1. **Per request:** `<intake-base>/intake/<date>-<slug>/decisions.md` (from
   `assets/decision-log.md`) — the full, readable record. For each
...
- **Output per request:** `<intake-base>/intake/<date>-<slug>/` containing
  ...
  `supporting/*.md`.
- **Templates:** this skill's own `assets/` folder (bundled alongside this
  `SKILL.md`).
- **PM memory:** location comes from `PROJECT-CONTEXT.md` if the project
  names one; otherwise this skill's own `references/` folder (a
