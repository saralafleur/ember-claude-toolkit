...
> `~/.cursor/skills/team-qa/...` path below means "the same-named folder
...
- **QA run-log:** location per `PROJECT-CONTEXT.md` if configured, else
  `~/.cursor/skills/team-qa/assets/qa-run-log.md` (past QA runs).
...
`~/.cursor/skills/team-qa/references/qa-assessment.md`) with:
...
- Append a row to the QA run-log (location per `PROJECT-CONTEXT.md`, else the
  global fallback at `~/.cursor/skills/team-qa/assets/qa-run-log.md`) — date,
