...
> `~/.cursor/skills/team-status/...` path below means "the same-named folder
...
`~/.cursor/skills/team-status/references/status-report.md`). It must contain:
...
`~/.cursor/skills/team-status/references/status-run-log.md` as a cross-project
