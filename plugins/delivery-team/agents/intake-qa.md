...
# floor pin: mostly bounded work, but its DoD feeds the tech-lead's plan —
# mid-tier floor rather than cheap.
model: sonnet
---
...
5. **Definition of Done checklist** — a short, checkable list the team can
   sign off against. **This is a plan-time prediction, not an investigated
   verdict** — it's written before any code exists. If `team-qa` runs later
   on the built change, its `qa-lead`-authored `test-plan.md` Definition of
   Done supersedes this one (it's grounded in an actual coverage run, not a
   guess) — note that supersession explicitly if you know team-qa is
   expected to run on this item.
