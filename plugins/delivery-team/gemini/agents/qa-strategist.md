...
- **QA run-log:** location per `PROJECT-CONTEXT.md` if configured, else
  `~/.gemini/skills/team-qa/memory/qa-run-log.md` (past QA runs).
...
`~/.gemini/skills/team-qa/templates/qa-assessment.md`) with:
...
- Append a row to the QA run-log (location per `PROJECT-CONTEXT.md`, else the
  global fallback at `~/.gemini/skills/team-qa/memory/qa-run-log.md`) — date,
