...
> standalone install (`~/.gemini/skills/team-status/` + `~/.gemini/agents/`).
> If you installed this as a Gemini CLI extension instead (`gemini extensions
> install ...`, landing under `.gemini/extensions/delivery-team/`), every
> `~/.gemini/skills/team-status/...` path below means "the same-named folder
> bundled alongside this `SKILL.md`", and `~/.gemini/agents/<name>.md` means
> "the matching file in this extension's own `agents/` folder" — same
> relative layout, different root.

> **How to invoke each role:** these are subagents bundled with this
> extension. Delegate to a subagent by name — either invoke it explicitly
> with `@<name> <task and inputs>` (e.g. `@status-scanner ...`), or describe
> the task and let automatic delegation match it to the subagent whose
> `description` fits (Gemini CLI's main agent does this on its own when a
> task matches a registered subagent). Always give the agent: the target
> folder, the output path for the report, and (for scanners) the specific
> item folder it owns. Note that subagents are a Gemini CLI preview feature —
> if delegation doesn't trigger reliably in your version, paste the role
> brief from `~/.gemini/agents/<name>.md` into a plain task instead.
...
and ask the user directly:** "Which folder should I assess — a whole batch,
or a single intake-base folder? I'll reconstruct the true current state and
write a `status-report.md` there." If the shape is genuinely ambiguous, state
the interpretation you're taking and proceed; don't stall on it.
...
- **Output per run:** `<target>/status-report.md` (+ transient
  ...
  was logged).
- **Templates:** `~/.gemini/skills/team-status/templates/` (or this
  extension's own `skills/team-status/templates/` under a plugin install —
...
- **Memory:** the status run-log location comes from `PROJECT-CONTEXT.md` if
  ...
  `~/.gemini/skills/team-status/memory/status-run-log.md` (a cross-project
