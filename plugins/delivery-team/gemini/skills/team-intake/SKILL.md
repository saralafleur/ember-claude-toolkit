...
> standalone install (`~/.gemini/skills/team-intake/` + `~/.gemini/agents/`).
> If you installed this as a Gemini CLI extension instead (`gemini extensions
> install <source>`, landing under `.gemini/extensions/delivery-team/`), every
> `~/.gemini/skills/team-intake/...` path below means "the same-named folder
...
> extension (`agents/<name>.md`). Delegate to one either by mentioning it
> explicitly — `@<name> <task + inputs>` (e.g. `@intake-architect ...`) — or
> by letting automatic delegation trigger it, since the main agent matches
> the task at hand to the subagent's `description`. Note that Gemini CLI
> subagents are a preview feature; if delegation misbehaves, fall back to
> running the task yourself using the role brief in `agents/<name>.md` as
> your instructions. Always give the agent: the request-brief path, the
> output dir, and the PM's classification once known.
...
- **Output per request:** `<intake-base>/intake/<date>-<slug>/` containing
  ...
  `supporting/*.md`.
- **Templates:** `~/.gemini/skills/team-intake/templates/`.
- **PM memory:** location comes from `PROJECT-CONTEXT.md` if the project
  names one; otherwise `~/.gemini/skills/team-intake/memory/` (a
