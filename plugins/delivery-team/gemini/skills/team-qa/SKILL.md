...
skill, ask for it before proceeding (see Step 0).
...
> standalone install (`~/.gemini/skills/team-qa/` + `~/.gemini/agents/`). If
> you installed this as a Gemini CLI extension instead, every
> `~/.gemini/skills/team-qa/...` path below means "the same-named folder
> bundled alongside this `SKILL.md`" (extensions install to
> `.gemini/extensions/<name>/`, keeping the same `skills/` and `agents/`
> subfolder layout), and `~/.gemini/agents/<name>.md` means "the matching
> file in this extension's own `agents/` folder" — same relative layout,
> different root.

> **How to invoke each role:** these are registered subagents. Delegate to one
> explicitly with `@<name> <task>` (e.g. `@qa-coverage-cartographer ...`), or
> let Gemini's automatic delegation trigger it since the main agent matches
> the task to the subagent's description — either way, always give the agent:
> the `change-brief.md` path, the output dir, and (once known) the coverage
> verdict. (If a subagent isn't available — e.g. subagent support isn't
> enabled in this Gemini CLI build — fall back to doing that role's work
> yourself, following the role brief in `~/.gemini/agents/<name>.md`.)
...
Delegate to these **four subagents in parallel**. Give
...
- **Output per run:** `<base>/qa/<date>-<slug>/` (or `<intake-dir>/qa/`) containing
  ...
  `supporting/*.md`.
- **Templates:** `~/.gemini/skills/team-qa/templates/`.
- **Memory:** the QA run-log location comes from `PROJECT-CONTEXT.md` if the
  project names one; otherwise falls back to
  `~/.gemini/skills/team-qa/memory/qa-run-log.md` (a cross-project log — less
