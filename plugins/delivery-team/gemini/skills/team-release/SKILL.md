...
> standalone install (`~/.gemini/skills/team-release/` + `~/.gemini/agents/`).
> If you installed this as a Gemini CLI extension instead, every
> `~/.gemini/skills/team-release/...` path below means "the same-named folder
> bundled alongside this `SKILL.md`" (extensions install to
> `.gemini/extensions/<name>/`, keeping the same relative layout), and
> `~/.gemini/agents/<name>.md` means "the matching file in this extension's
> own `agents/` folder" — same relative layout, different root.

> **How to invoke each role:** delegate to the named subagent — either invoke
> it explicitly with `@<name> <task>` (e.g. `@release-scribe ...`), or let
> Gemini's automatic delegation trigger it since the main agent matches the
> task to the subagent's `description`. Give each: the version label, the
> list of shipped items with their artifact paths, and the output dir. (If a
> name isn't available as a subagent, fall back to running the task yourself
> using the role brief in `~/.gemini/agents/<name>.md`.)
...
- Check `PROJECT-CONTEXT.md` for this project's version source of truth and
  ...
  user gave a version and/or folders, use them; otherwise ask the user
  directly: "What's in this release? Give me the version and the folder(s) or
  build(s) it covers."
...
- **Output per release:** `<release-docs-root>/<version>/` (per
  ...
  `decisions.md`.
- **Templates:** `~/.gemini/skills/team-release/templates/`.
- **Memory:** the release-log location comes from `PROJECT-CONTEXT.md` if the
  project names one; otherwise
  `~/.gemini/skills/team-release/memory/release-log.md` (a cross-project
