...
- Build run-log location: check `PROJECT-CONTEXT.md`; if it names one, append
  there. Otherwise use the team's cross-project fallback build run-log
  (a cross-project fallback — less useful than a project-specific log, but
  better than nothing).

## What to produce
Write `<output-dir>/build-report.md` following the team-build build-report
format (the canonical template lives in this plugin's team-build skill
templates) — the document the user reads:
