<!-- assumption: Agent Plugins (Preview) prompt files use YAML frontmatter with `description`, `name`, and an optional `tools` array; field names/shape may still change. -->
---
name: wrap-up
description: 'Close out the current unit of work in a git repo: audit for anything outstanding (uncommitted/untracked changes anywhere in the repo, PENDING/PARKED decisions, open caveats from a build/QA report, unfinished items raised earlier in the conversation), surface that list and get an explicit human go-ahead, then commit → push → merge into the repo''s actual default branch → clean up the now-merged branch/worktree. Invoke with /wrap-up, or on the phrases "let''s wrap this up", "wrap this up", "wrap it up", "time to wrap up". Never skips the human confirmation gate, even when nothing outstanding is found.'
...
Invoked via `/wrap-up`. Closes out a unit of work: **audit → human gate →
commit → push → merge → clean up.** It exists so that finishing a piece of
work is never a silent `git commit && push` — it's a deliberate checkpoint
where anything left undone gets said out loud before the branch disappears.

This is a **direct, single procedure** — no sub-agent delegation. Run every
step yourself, sequentially, using git and (optionally) `gh` in the terminal.

Treat the invocation as: *the current unit of work is what we've been doing
in this conversation — close it out.* If more than one repo/branch/worktree
is in play and the invocation named one, use that; otherwise infer it.
...
3. **Any `decisions.md` for this effort** — search for `PENDING` or `PARKED`
...
Append one line per wrap-up to this prompt's run-log at
`.github/memory/wrap-up-log.md` (create it with a header row if it doesn't
exist yet): date, project, repo path, branch merged, merge commit sha,
one-line summary of what shipped, and whether anything was deferred (from
the Step 2 gate). <!-- assumption: prompt files have no bundled companion-file convention like a Claude skill's memory/ folder, so the run-log path is spelled out explicitly here. -->

Ship that log **empty** (header only) — a fresh install should never inherit
anyone else's project history. Real entries are local instance data, not
shippable content; keep them out of any copy of this repo you intend to
publish or share.
