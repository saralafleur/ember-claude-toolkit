...
Invoke with an optional command name — e.g. `iphone-setup`, `docker-setup`,
`node-setup` — naming the environment to scaffold. Omit it and the skill
interviews for what the project needs.
...
Scaffold from `references/lifecycle.template.md` (one reference doc covering
all four actions) and `scripts/lifecycle-check.template.sh` (the shared
...
Scaffold it from `references/status.template.md`.
...
Copy and fill the templates that ship with this skill — the `.sh` audit
templates live in this skill's `scripts/` directory, the `.md` doc templates
in its `references/` directory:

| Template | Becomes | Fill in |
|---|---|---|
| `references/SKILL.template.md` | `.claude/skills/devops/SKILL.md` | project name, command rows, argument-hint |
| `references/reference.template.md` | `.claude/skills/devops/references/<command>.md` | audit keys, plan table, install steps, smoke test |
| `scripts/check.template.sh` | `.claude/skills/devops/scripts/<command>-check.sh` | one check block per prerequisite |
| `references/status.template.md` | `.claude/skills/devops/references/status.md` | usually verbatim — it is fully generic |
| `references/lifecycle.template.md` | `.claude/skills/devops/references/lifecycle.md` (one file covering all four `build`/`up`/`down`/`remove` actions) | naming the project's actual solutions/services and backend (Docker Compose, native PID-file processes, or mixed) |
| `scripts/lifecycle-check.template.sh` | `.claude/skills/devops/scripts/<name>-check.sh` (e.g. `compose-check.sh` or `native-check.sh`) | one built+running check block per solution, using whichever commented pattern (Docker or native) matches — only when the project has 2+ runnable units |
