...
Launch these **three agents in parallel** (one message, multiple tool
calls), regardless of mode — a technical fix can be raised by the UX lens
and vice versa, so classification happens later, not by restricting who
runs. Give each the `solution-brief.md` path and the `supporting/` output
path:
...
Launch `product-analyst-validator` **twice, in the same message**, both
...
at `~/.claude/skills/product-analyst/memory/run-log.md` (see `SKILL.md`'s
Path note if this is a plugin install). It:
