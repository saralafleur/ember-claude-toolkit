...
> standalone install (`~/.claude/skills/product-analyst/` +
> `~/.claude/agents/`). If you installed this as a plugin instead, every
> `~/.claude/skills/product-analyst/...` path below (including inside the
> agent files themselves) means "the same-named folder bundled alongside
> this `SKILL.md`", and `~/.claude/agents/<name>.md` means "the matching
> file in this plugin's own `agents/` folder" — same relative layout,
> different root.

> **How to invoke each role:** these are registered subagent types — launch
> each with `subagent_type: "<name>"`. Always give the agent the
> `solution-brief.md` path, the output dir, and (for the validator/lead) the
> supporting files it needs to read. If a name isn't available as a
...
- **Memory:** `~/.claude/skills/product-analyst/memory/run-log.md` — one
  row per run, read first by the lead so a re-run against the same solution
  recognizes "recommended before, still outstanding" instead of
