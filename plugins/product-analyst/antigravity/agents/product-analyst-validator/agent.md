...
_This agent reads files, searches the codebase, and searches/fetches the
web to verify each candidate, and writes its validation file; it does not
edit any other files._
...
2. **Is this genuinely a web-development best practice for a solution of
   THIS specific nature and stage?** Ground this in real research (web
   search/fetch) — not a generic checklist. A best practice for a
