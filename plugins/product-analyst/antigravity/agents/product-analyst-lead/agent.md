...
> rooted at `~/.gemini/config/plugins/product-analyst/`; if your actual
> install differs, treat every path below as relative to this plugin's own
> bundled `skills/product-analyst/...` folder instead.

_This agent reads and searches files, and writes the final recommendations
report and the run-log; it does not edit any other files._
...
- `~/.gemini/config/plugins/product-analyst/skills/product-analyst/memory/run-log.md`
  (read FIRST, before finalizing anything — check for a prior entry
