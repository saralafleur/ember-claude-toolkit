...
<!-- assumption: the source's WebSearch/WebFetch tools both map onto Copilot's single `fetch` tool, since Copilot's Preview toolset has no dedicated open web-search tool id. -->
...
2. **Is this genuinely a web-development best practice for a solution of
   THIS specific nature and stage?** Ground this in real research (the
   `fetch` tool) — not a generic checklist. A best practice for a
