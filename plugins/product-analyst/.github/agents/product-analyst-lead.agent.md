...
- The team's cross-project run-log at `.github/memory/run-log.md` (sibling
  to this plugin's `.github/agents/` folder — read FIRST, before finalizing
  anything, to check for a prior entry against this same target. If it
  doesn't exist yet, this is the first run against any target — treat it as
  empty, and create it with just the header row shown below on first write)
...
Append a row to `.github/memory/run-log.md` (create it with a header row —
`| Date | Mode | Target | Top (names) | Report link |` — if it doesn't
exist yet): date, mode (`product`/`tech`), target/solution identifier, the
final top 10 (names only), and a link to this run's `recommendations.md`.
Keep it terse — this is a lookup table for future runs, not a duplicate of
the report itself.
