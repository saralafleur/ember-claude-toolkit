...
tools: ['codebase', 'search', 'runCommands', 'fetch', 'editFiles']
...
<!-- assumption: the source's WebSearch/WebFetch tools both map onto Copilot's single `fetch` tool (search-by-URL-fetch), since Copilot's Preview toolset has no dedicated open web-search tool id. -->
...
2. Use the `fetch` tool to research what this specific class of solution
   typically includes today — don't rely on stale internal assumptions
   about "best practice" in the abstract.
