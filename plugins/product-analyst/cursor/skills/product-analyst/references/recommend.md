...
Launch these **three agents in parallel** (one message, multiple
delegations), regardless of mode — a technical fix can be raised by the
UX lens and vice versa, so classification happens later, not by
restricting who runs. Give each the `solution-brief.md` path and the
...
at `~/.cursor/skills/product-analyst/memory/run-log.md` (see `SKILL.md`'s
