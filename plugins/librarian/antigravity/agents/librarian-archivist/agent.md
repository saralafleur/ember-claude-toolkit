...
Typical capabilities needed: reading and searching files, plus writing and editing
files and running shell commands to create/move entries — you are the only agent
that writes to `<library-root>`, to the librarian's own files, and to
`config/library-locations.json`.
