...
Typical capabilities needed: reading files, searching across the library and audit
logs (grep/glob), and running read-only shell commands; the only thing you write is
a single proposal file — you never modify the library or the librarian's own files.
