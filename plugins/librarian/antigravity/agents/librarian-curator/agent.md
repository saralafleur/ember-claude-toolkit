...
Typical capabilities needed: reading files, searching across the repo and library
(grep/glob), and running read-only shell commands; the only thing you write is a
single proposal file to the output path you're given — you never write to the
library or the repo.
...
- How we accomplished something reusable: browser-testing flows, a CLI utility,
